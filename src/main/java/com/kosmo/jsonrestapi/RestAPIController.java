...
public class RestAPIController {
	...
	public Map<String, String> writeGet(BoardDTO boardDTO) {
		...
		//Mapper의 write메서드 호출. <insert 엘리먼트의 경우 항상 0,1을 반환>
	...
	/*
	React연동을 위한 추가메서드. 앞에서 작성한 메서드와 요청명은 동이랗나
	전송박식이 다르므로 각 요청에 따라 구분된다.
	 */
	@RequestMapping(value="/restapi/boardWrite.do", method=RequestMethod.POST)
	@ResponseBody
	/*
	React에서 fetch()함수를 통해 POST방식으로 요청한 데이터를 받을 때
	...
	public Map<String, String> writePost(@RequestBody String data, BoardDTO boardDTO) {
		...
		//json-simple의존설정 필요. 해당객체를 통해 JSON데이터를 파싱한다.
